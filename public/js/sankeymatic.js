...
function radio_value(radio_input_name) {
  ...
  Array.prototype.slice
    .call(document.getElementsByName(radio_input_name))
...
function render_png() {
  ...
  png_link_el.setAttribute(
    "download",
    "sankeymatic_" + png_w + "x" + png_h + ".png"
  );

  // update img tag hint
  img_tag_el.innerHTML =
    "<code>&lt;img width=&quot;<strong>" +
    orig_w +
...
function render_updated_outputs() {
...
function render_sankey(nodes_in, flows_in, config_in) {
  ...
  separators.thousands =
    typeof separators.thousands === "undefined" || separators.thousands === null
      ? ","
      : separators.thousands;
  separators.decimal =
    typeof separators.decimal === "undefined" || separators.decimal === null
      ? "."
      : separators.decimal;
  ...
  link = svg
    ...
    .style("stroke", function (d) {
      ...
      return d.color
        ? d.color
        : d.source.inherit_right
        ? d.source.color
        : d.target.inherit_left
        ? d.target.color
        : config_in.default_flow_inherit === "source"
        ? d.source.color
        : config_in.default_flow_inherit === "target"
        ? d.target.color
    ...
    .on("mouseover", function (d) {
      d3.select(this).style(
        "stroke-opacity",
        d.opacity_on_hover || (Number(config_in.default_flow_opacity) + 1) / 2
      );
    })
  ...
  if (config_in.show_labels) {
    link
      .append("title") // Make tooltips for FLOWS
      .text(function (d) {
        return (
  ...
  node
    ...
    .style({
      // be explicit about the font specs:
...
function process_sankey(inputNodeList) {
  ...
  function add_message(msg_class, msg_html, put_at_beginning) {
    ...
    messages_el.innerHTML = put_at_beginning
  ...
  good_flows.forEach(function (flow) {
    ...
    if (matches !== null) {
      ...
      matches = possible_color.match(/^#([0-9A-F]{3,6})?(\.\d{1,4})?$/i);
      if (matches !== null) {
  ...
  node_order.forEach(function (nodename) {
    ...
    inherit_left =
      this_node.inherit1 === "<<" || this_node.inherit2 === "<<" ? 1 : 0;
    inherit_right =
      this_node.inherit1 === ">>" || this_node.inherit2 === ">>" ? 1 : 0;
    readynode = {
      name: nodename,
  ...
  // Decimal:
  ["default_node_opacity", "default_flow_opacity", "curvature"].forEach(
    function (field_name) {
      var field_val = document.getElementById(field_name).value;
  ...
  node_order.forEach(function (nodename) {
    ...
    if (this_node.from_sum > 0 && this_node.to_sum > 0) {
      ...
      if (do_cross_checking && Math.abs(difference) >= epsilon_difference) {
  ...
  status_message =
    "Showing: <strong>" +
    approved_flows.length +
    " Flows</strong> between <strong>" +
    approved_nodes.length +
    " Nodes</strong>.";
  ...
  if (Math.abs(total_difference) < epsilon_difference) {
    ...
    status_message +=
      ...
      unit_fy(total_inflow) +
      "</strong>. Total <strong>OUT</strong> = <strong>" +
      unit_fy(total_outflow) +
      "</strong>.";
  }
  ...
  var tallest_node_height = parseFloat(
  ...
  var scale_report =
    unit_fy(max_node_val) +
    " / " +
    fix_separators(
      d3.format(",.2f")(tallest_node_height),
      approved_config.seps
    ) +
    "px = <strong>" +
    unit_fy(max_node_val / tallest_node_height, 6) +
    "/px</strong>";
